...
UnexpectedEvents.prototype.event = function () {
    throw new Error('event() called when not in expectation mode.');
};
...
function assertMultipleEvents(expect, subject, channel, expectedEvents) {
    return expect.promise(function (resolve, reject) {
    ...
    installInto: function (expect) {
        ...
        expect.addAssertion('<EventEmitter> for multiple events on <string> <assertion>', function (expect, subject, channel) {
            ...
            if (!validateArrayArgument(suppliedArgs, expectedEvents)) {
                return expect.promise.reject(new Error('An array of events is expected.'));
            }

            return assertMultipleEvents(expect, subject, channel, expectedEvents);
        });
        ...
        expect.addAssertion('<UnexpectedEvents> to have events satisfying <function>', function (expect, subject, recorder) {
            expect.errorMode = 'nested';

            var expectedCalls = [];
            subject.event = function () {
                var args = Array.prototype.slice.call(arguments);
                var argsWithoutChannel = args.slice(1);

                expectedCalls.push(argsWithoutChannel);
            };
            ...
            return expect(function () {
                ...
                var expectedEvents = new UnexpectedEvents(expectedCalls.map(function (call) {
                    return new UnexpectedEvent(call);
                }));

                return expect(subject, 'to satisfy', expectedEvents);
            }).finally(function () {
                delete subject.event;
            });
        });

        expect.addAssertion('<Promise> to have events satisfying <function>', function (expect, subject, recorder) {
            return subject.then(function (unexpectedEvents) {
                if (!(unexpectedEvents instanceof UnexpectedEvents)) {
                    throw new Error('Supplied promise did not resolve with UnexpectedEvents.');
                }
            });
        });
    },
